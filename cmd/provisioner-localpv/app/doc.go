/*
This package contains OpenEBS Dynamic Kubernetes Local PV provisioner
using external storage provisioner library:
https://github.com/kubernetes-sigs/sig-storage-local-static-provisioner

OpenEBS Local PV either be a sub directory or a complete storage device, 
both of which will be specified as hostpath in the PV spec.

Hostpath volume mount is one of the oldest supported option for Kubernetes
...
However, the use of hostpath by application developers has been discouraged
...
application developers from using hostpaths or at least restrict to only trusted
namespaces. For more, check out:
https://kubernetes.io/docs/concepts/policy/pod-security-policy/#volumes-and-file-systems

The Local PVs provide an viable alternative to using hostpaths,
while addressing the security risk of exposing the hostpaths to the
application developers. For example application developer can now create
PVCs - that request for hostpath based Local PV.

This Provisioner can dynamically satisfy the PVC request
by providing a PV object. The PV objects are not accessible by the
developers and hence they shield details about actual hostpaths from
developers.

While the primary motivation to add Hostpath Local PVs was to address the
security vulnerabilities, hostpaths are probably the only viable options
...
Hostpath PV vs Local PV:
- Multiple Hostpath PVs can be provisioned on a single underlying disk. Local PVs are designed to use up the entire disk.
- Hostpath PV only supports filesystem mounts. Local PVs can also support block mounts.

Hostpath PV vs native hostPath:
- Hostpath PV supports volume topology constraints, allowing the Pods using the Hostpath PV to 
...
- Hostpath PV supports enforcing checks on the paths used. (Can also be achieved via PSP, but 
  enforcing via StorageClass makes it easy to put all storage policies at one place.)
- Hostpath PV support backup/restore of data using tools like Heptio Velero
- Hostpath PV supports enforcing the Capacity limits. Hostpath PV can ve converted to 
  Read-Only when the Capacity limit is reached.
- Hostpath PV supports selecting a node with the available capacity. Can limit the number of 
  Hostpath PVs provisioned based on the capacity available on the node.

How it works:
The Local PV Dynamic Provisioner needs Kubernetes 1.12 storage features like 
Local Volume Topology, PV NodeAffinity parameters and so forth.

Local PV Provisioner uses the StorageClass `waitForConsumer` option, so it
- waits for the workload to be scheduled to a suitable node.
- on the selected node, a PV object is created either using a Hostpath or a StorageDevice.

HostPath is used if any of the following options are available in the StorageClass:
- BasePath: The Hostpath volumes will be created under `/var/openebs`.
  This default path can be changed by passing the "OPENEBS_IO_BASE_PATH" ENV
  variable to the Hostpath Provisioner Pod.
...
The location of the hostpaths can be controlled by the cluster administrator
...
Limitations:
- It is not possible to control the nodes where hostpaths can be created.
...
Future Work:
Hostpaths provide an easy (almost no hassle) way of running workloads. The build of the
